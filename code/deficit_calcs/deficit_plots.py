...
def single_site_fig(data, data_modis, show_modis = False, ppt = 'cum_prism_ppt', et_1 = 'cum_pml_ET', et_2 = 'cum_modis_ET', directory_name = np.nan):
   ...
   # for i in range(num_of_sites):
    ...
    axShallow.fill_between(plot_data['id'], 0, plot_data['cum_q_mm'], color='#4d8f53', label='Q (mm)')
    cf = axShallow.plot(plot_data['id'], plot_data[et_1],'--',color='#ED9935', alpha = 0.8, label = 'ET (PML)')
    ...
    if show_modis == 'True':
        ...
        # CUMULATIVE ET
        axShallow.plot(plot_modis['id'], plot_modis[et_2],'--',color='#612fa3', alpha = 0.8, label = 'ET (MODIS)')
